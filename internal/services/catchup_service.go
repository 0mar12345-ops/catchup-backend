...
func (s *CatchUpService) announcementToContentItem(ann googleAnnouncement, schoolID, courseID, ingestionJobID bson.ObjectID, now time.Time) models.ContentItem {
	...
	// Derive a meaningful title from the first non-empty line of the announcement text
	announcementTitle := "Announcement"
	if ann.Text != "" {
		lines := strings.Split(strings.TrimSpace(ann.Text), "\n")
		...
		if mat.DriveFile != nil {
			...
			if !isSupported {
				if mimeType == "application/octet-stream" || mimeType == "" {
					// Classroom API often returns application/octet-stream for uploaded DOCX/PPTX.
					// Mark as supported and resolve the real type via Drive metadata at extraction time.
					att.ExcludeCause = "MIME type unknown — will resolve via Drive API"
					att.IsSupported = true
					att.Kind = models.AttachmentKindOther // resolved at extraction time
				} else {
...
// AttachmentKindDOCX and AttachmentKindPPTX are virtual kinds used internally
// to route uploaded Office files to the correct extractor.
const (
	attachmentKindDOCX = models.AttachmentKind("docx")
	attachmentKindPPTX = models.AttachmentKind("pptx")
)
...
func classifyDriveFile(mimeType, fileName string) (models.AttachmentKind, bool) {
	...
	case strings.HasSuffix(fileName, ".docx"), strings.HasSuffix(fileName, ".doc"):
		return attachmentKindDOCX, true
	case strings.HasSuffix(fileName, ".pptx"), strings.HasSuffix(fileName, ".ppt"):
		return attachmentKindPPTX, true
	case strings.HasSuffix(fileName, ".jpg"), strings.HasSuffix(fileName, ".jpeg"),
		strings.HasSuffix(fileName, ".png"), strings.HasSuffix(fileName, ".gif"):
		return models.AttachmentKindImage, false
	case strings.HasSuffix(fileName, ".mp4"), strings.HasSuffix(fileName, ".avi"),
		strings.HasSuffix(fileName, ".mov"), strings.HasSuffix(fileName, ".wmv"):
		return models.AttachmentKindVideo, false
	}

	// Unknown MIME — mark as needing metadata resolution at extraction time
...
// extractYouTubeIDsFromText scans plain text for embedded YouTube URLs and returns video IDs.
// This handles cases where teachers paste YouTube links directly in announcement bodies
// rather than attaching them as proper YouTube attachments in Google Classroom.
var youtubeURLPatterns = []*regexp.Regexp{
	regexp.MustCompile(`(?:youtube\.com/watch\?(?:[^&\s"']*&)*v=|youtu\.be/)([a-zA-Z0-9_-]{11})`),
	regexp.MustCompile(`youtube\.com/embed/([a-zA-Z0-9_-]{11})`),
}

func extractYouTubeIDsFromText(text string) []string {
...
func (s *CatchUpService) extractTextFromAttachment(
	...
	// Always resolve the real MIME type from Drive for unknown/octet-stream attachments.
	// The Classroom API frequently returns application/octet-stream for uploaded DOCX/PPTX files.
	actualKind := att.Kind
	actualMimeType := att.MimeType
	if att.MimeType == "" || att.MimeType == "application/octet-stream" || att.Kind == models.AttachmentKindOther {
		...
		if err == nil && metadata.MimeType != "" {
			...
			fmt.Printf("[Drive] Resolved MIME for '%s': %s → kind: %s\n", att.Title, actualMimeType, actualKind)
		} else if err != nil {
			fmt.Printf("[Drive] Metadata fetch failed for '%s': %v\n", att.Title, err)
		}
	}
	...
	case attachmentKindDOCX:
		// Use Drive export API to convert uploaded DOCX → plain text server-side
		fmt.Printf("[Drive] Extracting DOCX as plain text: '%s'\n", att.Title)
		return s.extractViaExportAPI(ctx, client, att.ExternalID, "text/plain", att.Title)
	case attachmentKindPPTX:
		// Use Drive export API to convert uploaded PPTX → plain text server-side
		fmt.Printf("[Drive] Extracting PPTX as plain text: '%s'\n", att.Title)
		return s.extractViaExportAPI(ctx, client, att.ExternalID, "text/plain", att.Title)
	default:
		return "", fmt.Errorf("unsupported attachment kind: %s (mimeType: %s)", actualKind, actualMimeType)
	}
}
...
// extractViaExportAPI asks the Google Drive export endpoint to convert a file to the
// requested MIME type and returns the plain text. Works for Google Docs, Slides, and
// uploaded Office files (DOCX, PPTX) that Drive can convert.
func (s *CatchUpService) extractViaExportAPI(ctx context.Context, client *http.Client, fileID, exportMimeType, title string) (string, error) {
	exportURL := fmt.Sprintf("https://www.googleapis.com/drive/v3/files/%s/export?mimeType=%s",
		fileID, url.QueryEscape(exportMimeType))
	...
	if resp.StatusCode == http.StatusNotFound || resp.StatusCode == http.StatusBadRequest {
		// Drive cannot export this file type — return a useful marker instead of crashing
		body, _ := io.ReadAll(resp.Body)
		fmt.Printf("[Drive] Export not supported for '%s' (status %d): %s\n", title, resp.StatusCode, string(body))
		return fmt.Sprintf("[Attached document: %s — could not extract text, review the original file]", title), nil
	}
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return "", fmt.Errorf("drive export API error %d for '%s': %s", resp.StatusCode, title, string(body))
	}
	...
	fmt.Printf("[Drive] Exported '%s' as text: %d words\n", title, len(strings.Fields(text)))
	return text, nil
}
...
func (s *CatchUpService) extractAndCombineText(
		...
		for _, item := range contentItems {
			...
			for _, att := range item.Attachments {
				if !att.IsSupported {
					...
					default:
						// Log internally — not surfaced to UX
						fmt.Printf("[Content] Skipping unsupported attachment: %s (%s)\n", att.Title, att.Kind)
					}
					continue
				}
				...
				if err != nil {
					// Log internally — not surfaced to UX
					fmt.Printf("[Content] Failed to extract text from '%s': %v — skipping\n", att.Title, err)
					continue
				}
				if text != "" {
			...
			// Scan announcement text for YouTube URLs pasted inline (not as proper attachments).
			// Teachers often paste links directly in the announcement body.
			if item.Type == models.ContentTypeAnnouncement && item.Description != "" {
				// Collect video IDs already handled via attachments to avoid duplicates
				alreadyFetched := map[string]bool{}
				for _, att := range item.Attachments {
					if att.Kind == models.AttachmentKindVideo && att.ExternalID != "" {
						alreadyFetched[att.ExternalID] = true
					}
				}
				for _, videoID := range extractYouTubeIDsFromText(item.Description) {
					if alreadyFetched[videoID] {
						continue
					}
					fmt.Printf("[YouTube] Found embedded URL in announcement text, fetching transcript for video ID: %s\n", videoID)
					transcriptCtx, transcriptCancel := context.WithTimeout(ctx, 20*time.Second)
					transcript, hasTranscript, transcriptErr := s.fetchYouTubeTranscript(transcriptCtx, videoID)
					transcriptCancel()
					if transcriptErr != nil {
						fmt.Printf("[YouTube] Transcript error for embedded video %s: %v\n", videoID, transcriptErr)
					}
					if hasTranscript && transcript != "" {
						if len(transcript) > MaxYouTubeTranscriptChars {
							transcript = transcript[:MaxYouTubeTranscriptChars] + "... [transcript continues]"
						}
						itemParts = append(itemParts, fmt.Sprintf("[VIDEO_WITH_TRANSCRIPT: YouTube video %s]\n%s", videoID, transcript))
						fmt.Printf("[YouTube] Embedded video transcript fetched: %s (%d chars)\n", videoID, len(transcript))
					} else {
						itemParts = append(itemParts, fmt.Sprintf("[VIDEO_TITLE_ONLY: YouTube video %s]", videoID))
					}
				}
			}
	...
	if wordCount < MinWordCountThreshold {
		// Log internally — not surfaced to UX
		fmt.Printf("[Content] Word count (%d) below minimum threshold (%d)\n", wordCount, MinWordCountThreshold)
	}
...
==============================
CONTENT PRIORITY ORDER — follow strictly, top = highest priority:
==============================
  1. TEACHER ANNOUNCEMENTS — the teacher's exact words: topic, tasks, urgency, context (HIGHEST)
  2. WORKSHEETS / ASSIGNMENTS — tasks explicitly set by the teacher
  3. CORE LESSON MATERIALS — PDFs, PowerPoints, Google Docs
  4. VIDEO TRANSCRIPTS — enrich explanations with specific content
  5. SUPPLEMENTARY LINKS / SURVEYS — mention with link only, no deep analysis

==============================
STRICT RULES:
==============================

1. TEACHER ANNOUNCEMENT — copy the teacher's intent faithfully:
   - Read every sentence of the announcement. Do NOT paraphrase it into a generic topic summary.
   - The <h2>What Was Covered Today</h2> section MUST include ALL of the following if present:
       * The exact lesson context (e.g. "learning from home", "continuing from yesterday")
       * Every specific topic or activity mentioned by the teacher
       * Any urgency, deadline, or consequence the teacher stated (e.g. parent emails, task completion checks)
       * Any live session or meeting room mentioned
       * Revision phase / end-of-term context
   - Write this section as a faithful teacher-voice summary — imagine you are relaying exactly what the teacher communicated.
   - NEVER reduce this to a bland topic heading. The student must feel the full context of the day.

2. TASKS — extract and list every single one:
   - Scan the full announcement for every action item (watch video, complete worksheet, use Edrolo, etc.).
   - List EVERY task in <h2>Your Tasks</h2> as a numbered list.
   - For each task: clearly name it, describe what it covers, and include the link if available.
   - Surveys, forms, textbook questions — ALL must appear. Do NOT combine or skip any.

3. CONTENT FOCUS — suppress unrelated content:
   - If a supporting document covers a DIFFERENT topic from the main announcement (e.g. amino acids in a chemistry-synthesis lesson), do NOT include it in the main explanation.
   - Instead, add one line: "<p><em>Note: Additional materials on [topic] are available in your classroom for reference.</em></p>"
   - Only content that directly supports the announced lesson topic should appear in the main body.

4. CORE EXPLANATION — depth and connections:
   - For each key concept announced: explain WHAT it is, WHY it matters, and HOW it connects to other ideas.
   - Always link concepts: e.g. "The exothermic nature of the Haber Process (Le Chatelier) → low temperature favours yield → but slows rate → compromise temperature used → catalyst added to compensate → lower cost → industrial viability."
   - Use balanced equations, worked examples, and cause-effect chains.
   - Use <h3> sub-headings for each distinct concept.

5. EXTERNAL LINKS — every URL must appear:
   - ALL URLs from anywhere in the content MUST appear in <h2>Recommended Resources</h2>.
   - Format: <p><a href="URL">Descriptive title</a> — what it is and why it matters</p>
   - Google Docs, Edrolo, surveys, classroom links, YouTube — list ALL of them.
   - If a platform is named without a URL, still write: <p>Your teacher recommended <strong>[platform]</strong> — find it via your account.</p>

6. TEXTBOOK / LARGE REFERENCE DOCUMENTS:
   - [Large reference document] = big textbook. Do NOT summarise the whole book.
   - Direct the student to the SPECIFIC chapter(s) covering the announced topic.

7. VIDEO TRANSCRIPTS — mandatory, never skip:
   - [VIDEO_WITH_TRANSCRIPT: <title>] — you MUST use the transcript content.
     * ON-TOPIC video: integrate its key points into the main lesson explanation section.
     * OFF-TOPIC video (different subject): add a dedicated section:
         <h2>Additional Shared Resource: [exact video title]</h2>
         <p>Your teacher also shared this video on this day. Key points from it:</p>
         <ul><li>[point 1 from transcript]</li><li>[point 2]</li><li>[point 3]</li><li>[point 4]</li><li>[point 5]</li></ul>
   - [VIDEO_TITLE_ONLY: <title>]: list under Resources only.
   - The transcript text is in the input above — read it and use it. Do not claim it is unavailable.

8. SUPPORTING DOCUMENTS (PowerPoints, Success Criteria, Google Docs):
   - Only include content that is RELEVANT to the announced lesson topic.
   - Summarise as 3-5 bullet points per document under <h2>Supporting Materials</h2>.
   - Do NOT let supporting documents dominate or replace the teacher's announced content.

9. EXAMS / DEADLINES:
   - If the teacher mentioned exams, mock exams, task completion checks, or parent notifications → include <h2>Upcoming Assessments &amp; Deadlines</h2> with exact details.

==============================
OUTPUT STRUCTURE — use exactly this order:
==============================

<h2>What Was Covered Today</h2>
  Faithful, detailed summary of the teacher's full announcement. Include: learning-from-home context,
  continuation from previous lesson, ALL tasks set, urgency/deadlines, live session info, revision phase.
  This must read like a real teacher update — NOT a generic topic title.

<h2>[Main Topic Name]</h2>
  Deep explanation of the core lesson concepts. Use <h3> for each sub-topic.
  Integrate on-topic video transcript content here.
  Reference specific textbook chapters by topic name.

<h2>Supporting Materials</h2>  ← only if relevant supporting documents are present
  Key ideas from each document (3-5 bullets, labelled by document name).
  Exclude documents that cover unrelated topics.

<h2>Additional Shared Resource: [Video Title]</h2>  ← one per off-topic video with transcript
  5 specific bullet points from the transcript.

<h2>Your Tasks</h2>
  Numbered list of every task the teacher set. Include links for each.

<h2>Recommended Resources</h2>
  ALL URLs as <a href="..."> links with descriptions.

<h2>Upcoming Assessments &amp; Deadlines</h2>  ← only if mentioned by teacher
  Exact details.
...
Title: max 60 chars, must reflect the actual announced topic — not a generic subject name.

Learning objectives: 5-7, tightly aligned with the announced lesson content.

Quiz — 5-7 questions:
  - Questions 1-4: test core lesson concepts from the announcement and worksheet
  - Question 5 (REQUIRED if any [VIDEO_WITH_TRANSCRIPT:] is present): MUST test a specific fact from the video transcript. Start with "According to the shared video on [topic],..."
  - Questions 6-7 (optional): from supporting documents
  - Mix of MCQ (4 options) and short answer. Every question needs a correct answer.
